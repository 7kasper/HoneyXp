...
public class ExperienceManager {
	...
	public int getLevelForExp(int exp) {
		...
		if (exp > xpTotalToReachLevel[xpTotalToReachLevel.length - 1]) {
			...
			if (newMax > hardMaxLevel) throw new IllegalArgumentException("Level for exp " + exp + " > hard max level " + hardMaxLevel);
	...
	public int getXpForLevel(int level) {
		if (level < 0 || level > hardMaxLevel) throw new IllegalArgumentException("Invalid level " + level + "(must be in range 0.." + hardMaxLevel + ")");
